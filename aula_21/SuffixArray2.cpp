...
int main(int argc, char const *argv[])
{
    ...
    for (int i = 0; i < suffix_array.size(); i++)
        cout << suffix_array[i] << " " << s.substr(suffix_array[i]) << endl;
    cout << endl;

    vector<int> lcp = lcp_construction(s, suffix_array);
    for (int i = 0; i < lcp.size(); i++)
        cout << lcp[i] << " " << s.substr(suffix_array[i], lcp[i]) << endl;
    
    cout << "Number of unique substrings: " << unique_substrings(s, lcp) << endl;
